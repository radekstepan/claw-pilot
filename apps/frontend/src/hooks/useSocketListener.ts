...
export function useSocketListener() {
  ...
  useEffect(() => {
    ...
    const handleReconnect = (attemptNumber: number) => {
      console.log(
        `Socket reconnected after ${attemptNumber} attempt(s) — re-syncing state`,
      );
      const { lastSyncAt, syncData, fetchInitialData } =
    ...
    socket.on("gateway_status", ({ online, pairingRequired, deviceId }) => {
      console.log("Socket event: gateway_status", {
        online,
        pairingRequired,
        deviceId,
      });
    ...
    socket.on("agent_config_generated", ({ requestId, config }) => {
      console.log("Socket event: agent_config_generated", {
        requestId,
        config,
      });
      useMissionStore
        .getState()
        .resolveAgentConfig(requestId, config as GeneratedAgentConfig);
    });

    socket.on("agent_config_error", ({ requestId, error }) => {
      console.log("Socket event: agent_config_error", { requestId, error });
      toast.error(`Agent config generation failed: ${error}`, {
        duration: 8000,
      });
