...
function NotificationRow({ n, onClick }: NotificationRowProps) {
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-3 border-b border-black/[0.04] dark:border-white/[0.04] last:border-b-0 ${!n.read ? "bg-[var(--accent-500)]/[0.06]" : ""} ${isClickable ? "cursor-pointer hover:bg-black/[0.02] dark:hover:bg-white/[0.02]" : ""}`}
      onClick={onClick}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[9px] text-slate-400 dark:text-slate-600 mt-0.5 font-mono">
          {formatRelativeTime(n.timestamp)}
        </p>
      </div>
      {!n.read && (
        <span
          className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[var(--accent-500)] flex-shrink-0"
          aria-hidden="true"
        />
      )}
    </div>
...
export function NotificationsPanel({
  ...
  const markAllNotificationsRead = useMissionStore(
