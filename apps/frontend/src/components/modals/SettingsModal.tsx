...
export const SettingsModal = ({
  ...
  const [agentFormMode, setAgentFormMode] = useState<"create" | "edit">(
    "create",
  );
  ...
  const [confirmDeleteAgent, setConfirmDeleteAgent] = useState<Agent | null>(
    null,
  );
  ...
  const [importPreview, setImportPreview] = useState<RecurringTask[] | null>(
    null,
  );
      ...
      api
        ...
        .then((data: GatewayStatus) => {
          if (data.status === "ONLINE") {
            ...
            setGatewayLogs(
              `[System] Device pairing required.\nDevice ID: ${data.deviceId ?? "unknown"}\n\n${data.instructions ?? ""}\n`,
            );
          } else {
            setGatewayLogs(
              `[System] Gateway status: ${data.status}${data.error ? "\nError: " + data.error : ""}\n`,
            );
          }
        })
        .catch((err: unknown) => {
  ...
  const renderAgentsTab = () => (
    <div className="space-y-4 animate-fadeIn">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[10px] uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500">
          Active Squad
        </h3>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {agents.map((agent) => (
          <div
            key={agent.id}
            className="p-3 bg-slate-50 dark:bg-white/[0.02] border border-black/[0.05] dark:border-white/[0.05] rounded group hover:border-[var(--accent-scroll-hover)] transition-all"
          >
  ...
  const renderModelsTab = () => (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => api.getModels().then(setModels).catch()}
          className="text-[9px] uppercase font-bold text-[var(--accent-600)] dark:text-[var(--accent-400)] flex items-center gap-1"
        >
      ...
      <div className="space-y-2">
        {models.map((model) => (
          <div
            key={model.id}
            className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded"
          >
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded">
            <div>
              ...
              <p className="text-[9px] text-slate-400 mt-0.5">
                Used for buttons, active states and focus rings throughout the
                UI
              </p>
            </div>
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div
            key={m.label}
            className="p-3 bg-slate-50 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded text-center"
          >
            <div className="text-[8px] uppercase text-slate-400 dark:text-slate-500 mb-1">
              {m.label}
            </div>
      ...
      <div className="p-4 border accent-active-bg rounded space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          />
          <h3 className="text-[10px] uppercase font-bold text-slate-900 dark:text-white tracking-widest">
            Gateway Configuration
          </h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-[8px] uppercase font-bold text-slate-400 dark:text-slate-500">
              Gateway URL
            </label>
            <input
              type="text"
              ...
              onChange={(e) =>
                setConfig((c) => ({ ...c, gatewayUrl: e.target.value }))
              }
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) =>
                setConfig((c) => ({
                  ...c,
                  apiPort: parseInt(e.target.value, 10) || c.apiPort,
                }))
              }
              className="w-full bg-white dark:bg-black/20 border border-black/10 dark:border-white/10 rounded px-2 py-1.5 text-[10px] font-mono text-slate-900 dark:text-slate-300 outline-none focus:border-[var(--accent-500)]"
            />
          ...
          <div className="flex items-center gap-2 col-span-full">
            <input
              ...
              onChange={(e) =>
                setConfig((c) => ({ ...c, autoRestart: e.target.checked }))
              }
            />
            <label
            ...
            >
              Auto-restart OpenClaw gateway on CLI crash
            </label>
          </div>
        </div>
        <button
  ...
  const renderBackupTab = () => {
    const handleExport = async () => {
      setIsExporting(true);
      try {
        const tasks = await api.exportRecurringTasks();
        ...
        a.href = url;
        a.download = `recurring-tasks-backup-${new Date().toISOString().slice(0, 10)}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        toast.success(`Exported ${tasks.length} recurring task(s)`);
      } catch (err) {
        toast.error("Failed to export recurring tasks");
        console.error(err);
      } finally {
    ...
    const handleImport = async () => {
      ...
      setIsImporting(true);
      try {
        const result = await api.importRecurringTasks(importPreview);
        ...
        if (errors && errors.length > 0) {
          ...
          toast.success(
            `Imported ${imported} recurring task(s), skipped ${skipped}`,
          );
        } else {
          toast.info(
            `Skipped ${skipped} recurring task(s) - all already exist`,
          );
        }
        ...
        toast.error("Failed to import recurring tasks");
        console.error(err);
      } finally {
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="p-4 bg-slate-50 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded space-y-4">
          <div className="flex items-center gap-2 mb-2">
            <Download
              size={14}
              className="text-[var(--accent-600)] dark:text-[var(--accent-500)]"
            />
            <h4 className="text-[10px] uppercase font-bold text-slate-900 dark:text-white tracking-widest">
              Export Templates
            </h4>
          </div>
          <p className="text-[9px] text-slate-600 dark:text-slate-400">
            Download all recurring task templates as a JSON file for backup or
            transfer.
          </p>
          <button
        ...
        <div className="p-4 bg-slate-50 dark:bg-white/[0.02] border border-black/5 dark:border-white/5 rounded space-y-4">
          ...
          <p className="text-[9px] text-slate-600 dark:text-slate-400">
            Import recurring task templates from a backup file. Existing
            templates with the same ID will be skipped.
          </p>
          ...
          {!importPreview ? (
            <div className="relative">
              <input
                ...
                className="block w-full text-[9px] text-slate-600 dark:text-slate-400
                                    file:mr-3 file:py-1.5 file:px-3
                                    file:rounded file:border-0
                                    file:text-[9px] file:font-bold
                                    file:bg-[var(--accent-600)] file:text-white
                                    hover:file:bg-[var(--accent-500)]
                                    cursor-pointer
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 p-2 bg-white dark:bg-black/20 border border-black/10 dark:border-white/10 rounded">
                <FileText size={14} className="text-slate-400" />
                <span className="text-[9px] font-mono text-slate-600 dark:text-slate-400">
                  {importFile?.name}
                </span>
                <Badge variant="default">
                  {importPreview.length} templates
                </Badge>
              </div>
              ...
              <button
                ...
                className="text-[9px] text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        <ConfirmDialog
          open={confirmImport}
          title="Import recurring task templates?"
          message={`Import ${importPreview?.length || 0} template(s) from backup file? Any templates with IDs that already exist will be skipped.`}
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-3xl bg-white dark:bg-[#0c0a14] border border-black/10 dark:border-white/10 shadow-2xl flex flex-col h-[600px] animate-fadeIn overflow-hidden rounded">
        ...
        <div className="h-12 border-b border-black/[0.06] dark:border-white/[0.06] flex items-center px-6 justify-between bg-slate-50 dark:bg-white/[0.01] flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-900 dark:hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-500)] rounded"
          >
            <X size={18} />
          </button>
        </div>
