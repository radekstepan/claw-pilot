...
import { z } from 'zod';
import { CreateRecurringPayloadSchema, RecurringTaskSchema, RecurringTask } from '@claw-pilot/shared-types';
...
const recurringRoutes: FastifyPluginAsyncZod = async (fastify) => {
    ...
    fastify.post('/', { schema: { body: CreateRecurringSchema } }, async (request, reply) => {
        ...
        const validation = validateRecurringScheduleInput(body.schedule_type, body.schedule_value);
        if (!validation.valid) {
    ...
    fastify.patch('/:id', { schema: { body: UpdateRecurringSchema } }, async (request, reply) => {
        ...
        const nextScheduleValue = body.schedule_value !== undefined ? body.schedule_value : existing.schedule_value;
        const validation = validateRecurringScheduleInput(nextScheduleType, nextScheduleValue);
        if (!validation.valid) {
            return reply.status(400).send({ error: validation.error });
        }
