...
const deliverableRoutes: FastifyPluginAsyncZod = async (fastify, opts) => {
  fastify.patch("/:id/complete", async (request, reply) => {
    ...
    const allTasks = db.select().from(tasksTable).where(notArchived).all();
    ...
    // Toggle status.
    foundDeliverable.status =
      foundDeliverable.status === "COMPLETED" ? "PENDING" : "COMPLETED";

    const now = new Date().toISOString();
    const taskRow = db
      .select()
      .from(tasksTable)
      .where(and(eq(tasksTable.id, ownerTaskId), notArchived))
      .get()!;
    const deliverables = taskRow.deliverables ?? [];
  ...
  fastify.patch(
    ...
    async (request, reply) => {
      ...
      const taskRow = db
        .select()
        .from(tasksTable)
        .where(and(eq(tasksTable.id, taskId), notArchived))
        .get();
      ...
      if (!ids.every((id) => taskDeliverableIds.has(id))) {
        return reply.status(400).send({
          error: "One or more deliverable IDs do not belong to this task",
        });
      }
