...
const chatRoutes: FastifyPluginAsyncZod = async (fastify, opts) => {
  ...
  fastify.get(
    "/",
    { schema: { querystring: CursorPageQuerySchema } },
    async (request, reply) => {
        ...
        rows = db
          ...
          .where(
            or(
              ...
              and(
                eq(chatTable.timestamp, cursorTs),
                lt(chatTable.id, cursorId),
              ),
            ),
          )
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      enqueueAiJob(
        ...
        agentId === "main" ? "__gateway__" : agentId,
      );
    },
  );
