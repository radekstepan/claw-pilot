...
import { getAgents, getLiveSessions, gatewayCall, GatewayOfflineError } from '../openclaw/cli.js';
...
const systemRoutes: FastifyPluginAsync = async (fastify, opts) => {
    fastify.get('/stats', async (request, reply) => {
        ...
        try {
            ...
            if (gwErr instanceof GatewayOfflineError) {
                gatewayOnline = false;
        ...
        return reply.send({ activeAgents, totalAgents, tasksInQueue, completedToday, gatewayOnline });
    });
