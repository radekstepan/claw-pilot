...
async function driveHandshake(
  ...
  if (!ws.handshakeDriven) {
    ...
    ws.receive({
      ...
      id: connectFrame.id,
      ok: true,
      payload: { server: { version: "2026.2.0" } },
    });
...
describe("gatewayCall", () => {
  ...
  it("rejects when the connect response has ok:false", async () => {
    ...
    await expect(promise).rejects.toThrow(
      "Gateway connect failed: Not authorized",
    );
  });
  ...
  it("rejects when the method response has ok:false", async () => {
    ...
    await expect(promise).rejects.toThrow(
      "Gateway RPC 'sessions.list' failed: Unknown method",
    );
  });
...
describe("getAgents", () => {
  ...
  it("returns agents from payload.config.agents (array shape)", async () => {
    ...
    await driveHandshake({
      config: {
        agents: [{ id: "alpha", name: "Alpha", capabilities: ["code"] }],
      },
    });
...
describe("getLiveSessions", () => {
  ...
  it("returns sessions from an array-shaped payload", async () => {
    ...
    await driveHandshake([
      { key: "mc-gateway:gateway:main", status: "IDLE" },
  ...
  it("returns sessions from a { sessions: [...] } shaped payload", async () => {
    const promise = getLiveSessions();
    await driveHandshake({
      sessions: [{ key: "mc:mc-coder:main", status: "WORKING" }],
    });
    const sessions = await promise;
...
describe("routeChatToAgent", () => {
  ...
  it("uses the gateway main session for agentId 'main'", async () => {
    ...
    expect((ws1.sent[1]!.params as Record<string, unknown>).key).toBe(
      "mc-gateway:gateway:main",
    );
  ...
  it("uses mc:mc-{agentId}:main for other agent IDs", async () => {
    ...
    // sessions.patch
    await driveHandshake({});
    const patchParams = mockWsAll[0]!.sent[1]!.params as Record<
      string,
      unknown
    >;
  ...
  it("generates a unique idempotencyKey per call", async () => {
    ...
    await driveHandshake({});
    await driveHandshake({});
    const key1 = (mockWsAll[0]!.sent[2]!.params as Record<string, unknown>)
      .idempotencyKey;
    ...
    await driveHandshake({});
    await driveHandshake({});
    const key2 = (mockWsAll[0]!.sent[2]!.params as Record<string, unknown>)
      .idempotencyKey;
...
describe("spawnTaskSession", () => {
  ...
  it("patches the session then sends the prompt with deliver:true", async () => {
    const promise = spawnTaskSession(
      "worker-1",
      "task-42",
...
describe("createAgent", () => {
  ...
  it("creates agent and applies model and capabilities", async () => {
    const promise = createAgent("new-agent", "/workspace/agent", "gpt-4o", [
      "code",
      "review",
    ]);

    // First call: agents.create
    await driveHandshake({});
    const ws = mockWsAll[0]!;
    expect(ws.sent[1]!.method).toBe("agents.create");
    expect(ws.sent[1]!.params).toMatchObject({
      name: "new-agent",
      workspace: "/workspace/agent",
    });

    // Second call: config.get (for updateAgentMeta)
    await driveHandshake({
      config: { agents: { list: [{ id: "new-agent", name: "New Agent" }] } },
    });
    // Third call: config.patch
    await driveHandshake({});

    await expect(promise).resolves.toBeUndefined();
  });

  it("continues when agent already exists (non-fatal error)", async () => {
    const promise = createAgent("existing-agent", "/workspace/existing");

    // First call: agents.create - fails with "already exists"
    const ws = mockWsLatest!;
    ws.readyState = 1;
    ws.emit("open");
    ws.receive({
    ...
    // Should not throw - continues to updateAgentMeta
    await vi.advanceTimersByTimeAsync(0);
    await driveHandshake({});
  ...
  it("throws on gateway pairing required error", async () => {
    const promise = createAgent("new-agent", "/workspace/agent");
    ...
    // Wait for the connect response to be processed and agents.create to be sent
    await vi.advanceTimersByTimeAsync(0);

    // Now agents.create should have been sent - respond with pairing required error
    const agentsFrame = ws.sent[1]!;
    ws.receive({
      type: "res",
      id: agentsFrame.id,
      ok: false,
    ...
    await expect(promise).rejects.toThrow("pairing required");
  });

  it("throws on gateway offline error", async () => {
    const promise = createAgent("new-agent", "/workspace/agent");
    ...
    // Connection error on agents.create
    ws.emit("error", new Error("ECONNREFUSED"));
...
describe("updateAgentMeta", () => {
  ...
  it("updates name only", async () => {
    const promise = updateAgentMeta("agent-x", { name: "New Name" });

    // config.get
    await driveHandshake({
      config: {
        agents: {
          list: [{ id: "agent-x", name: "Old Name", workspace: "/ws" }],
        },
      },
    });
    // agents.update
    ...
    await expect(promise).resolves.toBeUndefined();
    const updateParams = mockWsAll[0]!.sent[2]!.params as Record<
      string,
      unknown
    >;
    expect(updateParams.name).toBe("New Name");
  });

  it("updates model only", async () => {
    const promise = updateAgentMeta("agent-x", { model: "gpt-4o" });

    // config.get (for hash and list) - hash at top level for the raw payload
    await driveHandshake({
      config: { agents: { list: [{ id: "agent-x", name: "Agent X" }] } },
      hash: "abc123",
    });
    // config.patch
    await driveHandshake({});
    ...
    await expect(promise).resolves.toBeUndefined();
    const patchParams = mockWsAll[0]!.sent[2]!.params as Record<
      string,
      unknown
    >;
    expect(patchParams.baseHash).toBe("abc123");
    expect(JSON.parse(patchParams.raw as string)).toMatchObject({
      agents: { list: [{ id: "agent-x", model: "gpt-4o" }] },
    });
  });

  it("updates capabilities only (maps to tools.allow)", async () => {
    const promise = updateAgentMeta("agent-x", {
    ...
    const patchParams = mockWsAll[0]!.sent[2]!.params as Record<
      ...
      unknown
    >;
    const raw = JSON.parse(patchParams.raw as string);
    expect(raw.agents.list[0].tools.allow).toEqual(["code", "review"]);
    expect(raw.agents.list[0].capabilities).toBeUndefined();
  });

  it("updates name, model, and capabilities together", async () => {
    const promise = updateAgentMeta("agent-x", {
      name: "New Name",
      model: "claude-3",
      capabilities: ["code"],
    ...
    // First config.get (for name update)
    await driveHandshake({
      config: {
        agents: {
          list: [{ id: "agent-x", name: "Old Name", workspace: "/ws" }],
    ...
    // Second config.get (for model/capabilities)
    await driveHandshake({
      config: {
        agents: {
          list: [{ id: "agent-x", name: "Old Name", workspace: "/ws" }],
  ...
  it("returns early when no fields to update", async () => {
    ...
    // Should resolve immediately without any gateway calls
    await expect(promise).resolves.toBeUndefined();
    expect(mockWsLatest).toBeNull();
  });

  it("returns early when agent not found in config list", async () => {
    const promise = updateAgentMeta("nonexistent", { model: "gpt-4o" });

    // config.get returns list without the agent
    await driveHandshake({
...
describe("getAgentFile", () => {
  ...
  it("returns content from payload", async () => {
    const promise = getAgentFile("agent-x", "SOUL.md");
    await driveHandshake({ content: "My soul content" });

    const result = await promise;
    expect(result).toBe("My soul content");
  });

  it("returns empty string when file.missing is true", async () => {
    const promise = getAgentFile("agent-x", "SOUL.md");
    await driveHandshake({ file: { missing: true } });

    const result = await promise;
    expect(result).toBe("");
  });

  it("returns empty string on gateway error (graceful degradation)", async () => {
    const promise = getAgentFile("agent-x", "SOUL.md");
    mockWsLatest!.emit("error", new Error("gateway error"));
...
describe("getAgentWorkspaceFiles", () => {
  beforeEach(() => {
    __resetGatewayClientForTest();
    mockWsLatest = null;
    mockWsAll = [];
  ...
  it("returns empty strings when no files exist", async () => {
    const listPromise = getAgentWorkspaceFiles("agent-x");

    await driveHandshake({ files: [] });
...
describe("setAgentFiles", () => {
  ...
  it("sets single file", async () => {
    const promise = setAgentFiles("agent-x", { soul: "My SOUL" });

    await driveHandshake({});
    await expect(promise).resolves.toBeUndefined();

    const params = mockWsAll[0]!.sent[1]!.params as Record<string, unknown>;
    expect(params.name).toBe("SOUL.md");
    expect(params.content).toBe("My SOUL");
  });

  it("throws on gateway pairing required error", async () => {
    const promise = setAgentFiles("agent-x", { soul: "test" });
...
describe("deleteAgent", () => {
  ...
  it("deletes agent with deleteFiles: true (default)", async () => {
    const promise = deleteAgent("agent-x", true);

    await driveHandshake({ deleted: true });

    const result = await promise;
    expect(result).toEqual({ deleted: true });

    const params = mockWsAll[0]!.sent[1]!.params as Record<string, unknown>;
    expect(params.agentId).toBe("agent-x");
    expect(params.deleteFiles).toBe(true);
  });

  it("deletes agent with deleteFiles: false", async () => {
    const promise = deleteAgent("agent-x", false);

    await driveHandshake({ deleted: true });

    const params = mockWsAll[0]!.sent[1]!.params as Record<string, unknown>;
    expect(params.deleteFiles).toBe(false);
