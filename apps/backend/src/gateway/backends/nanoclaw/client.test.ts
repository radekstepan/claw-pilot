...
import { NanoClawClient } from './client.js';
...
describe('NanoClawClient', () => {
    ...
    it('should handle successful GET requests', async () => {
        mockFetch.mockResolvedValueOnce({
            ok: true,
            status: 200,
            json: async () => ([{ id: '1', name: 'Agent 1', workspace: 'main' }])
        } as Response);
        ...
        expect(agents).toEqual([{ id: '1', name: 'Agent 1', workspace: 'main' }]);
    });
    ...
    it('should handle successful POST requests with NanoClaw-native format', async () => {
        mockFetch.mockResolvedValueOnce({
            ok: true,
            status: 201,
            json: async () => ({ id: '2', name: 'New Agent', workspace: 'main' })
        } as Response);
        ...
        expect(newAgent).toEqual({ id: '2', name: 'New Agent', workspace: 'main' });
    });
    ...
    it('should update agent with NanoClaw-native format', async () => {
        mockFetch.mockResolvedValueOnce({
            ok: true,
            status: 200,
            json: async () => ({ id: '1', name: 'Updated', workspace: 'main' })
        } as Response);
    ...
    it('should update agent with claw-pilot format (translates workspace)', async () => {
        mockFetch.mockResolvedValueOnce({
            ok: true,
            status: 200,
            json: async () => ({ id: '1', name: 'Updated', workspace: 'claw_staging' })
        } as Response);
